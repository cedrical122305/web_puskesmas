    ...
    if ($statusUpload == 0) {
        ...
        if (mysqli_num_rows($select) > 0) {
            ...
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
                $query = mysqli_query($conn, "INSERT INTO tb_daftar_obat (foto, nama_obat, kategori_obat, jenis_obat, satuan) values('" . $kode_rand . $_FILES['foto']['name'] . "','$nama_obat', '$kat_obat', '$jen_obat', '$sat_obat')");
                if ($query) {
